 ...
 * scrape-mughal.js
 *
 * Scrapes Mughal Empire coins from Zeno.ru.
 *
 * Category discovery: If the default cat=385 doesn't have Mughal coins,
 * run with --discover to walk categories 370-400 and print titles.
 *
 * Usage:
 *   node scripts/scrape-mughal.js --test           # 5 coins, print samples
 *   node scripts/scrape-mughal.js --run             # full run
 *   node scripts/scrape-mughal.js --resume          # continue from checkpoint
 *   node scripts/scrape-mughal.js --discover        # find correct category ID
 *   node scripts/scrape-mughal.js --cat 392 --test  # test with a specific cat ID
 */
...
const OUTPUT_FILE    = path.join(SCRIPTS_DIR, '..', 'src', 'data', 'mughal_coins.json');
const PROGRESS_FILE  = path.join(SCRIPTS_DIR, 'mughal_progress.json');
...
// ── ZenoSession (identical to scrape-sasanian.js) ─────────────────────────────
...
function parseGalleryPage(html, catId, currentPage) {
  ...
  // Extract category titles for discovery mode
  const titleRe = /showgallery\.php\?cat=(\d+)[^>]*>([^<]{3,60})</g;
  const catTitles = {};
  while ((m = titleRe.exec(html)) !== null) {
    catTitles[m[1]] = stripHtml(m[2]).trim();
  }
  return { subcats, coinIds, nextPage, catTitles };
}
...
async function discoverCategories(session) {
  ...
  console.log('Scanning categories 370–410 and 780–810 for Mughal/Delhi/India...\n');
  const rangeA = Array.from({ length: 41 }, (_, i) => 370 + i);
  const rangeB = Array.from({ length: 31 }, (_, i) => 780 + i);
  const toScan = [...rangeA, ...rangeB];
  for (const catId of toScan) {
    const url  = `${BASE_URL}/showgallery.php?cat=${catId}`;
    let html;
    try { html = await session.fetch(url); } catch { continue; }
    if (!html || html.length < 500) continue;
    const titleM = html.match(/<title>([^<]+)<\/title>/i);
    const title  = titleM ? stripHtml(titleM[1]) : '';
    const photoCount = (html.match(/showphoto\.php\?photo=\d+/g) || []).length;
    const folderCount = (html.match(/folder\.png/g) || []).length;
    if (title && title !== 'ZENO.RU') {
      const lower = title.toLowerCase();
      const relevant = lower.includes('mughal') || lower.includes('moghul') || lower.includes('india') ||
                       lower.includes('delhi') || lower.includes('sultanate') || lower.includes('islamic') ||
                       lower.includes('medieval') || photoCount > 0 || folderCount > 0;
      if (relevant) {
        console.log(`  cat=${catId}: "${title}" — photos: ${photoCount}, folders: ${folderCount}`);
      }
    }
  }
  console.log('\nDiscovery complete. Use --cat <ID> to test a specific category.');
}
...
async function main() {
  if (!TEST_MODE && !FULL_RUN && !RESUME_MODE && !CUSTOM_LIMIT && !DISCOVER && !CUSTOM_CAT) {
    ...
    console.log('  node scripts/scrape-mughal.js --resume         # resume from checkpoint');
    process.exit(0);
  }
  ...
  if (TEST_MODE) {
    console.log(`\nTest complete. Run with --run to scrape all Mughal coins.`);
    console.log(`If results look wrong, run --discover to find the correct category ID.`);
  }
}

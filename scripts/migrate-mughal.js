/**
 * Migrate Mughal coins from mughal_coins.json to Supabase coins table.
 * Usage: node scripts/migrate-mughal.js [--dry-run]
 *
...
async function main() {
  if (!fs.existsSync(JSON_FILE)) {
    ...
    console.error('   Run "node scripts/scrape-mughal.js --run" first to generate the data.');
    process.exit(1);
  }

  const raw   = fs.readFileSync(JSON_FILE, 'utf8');
  const coins = JSON.parse(raw);
  console.log(`📦 Loaded ${coins.length} Mughal coins from ${JSON_FILE}`);

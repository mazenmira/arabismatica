 ...
 * scrape-delhi.js
 *
 * Scrapes Delhi Sultanate coins from Zeno.ru.
 *
 * Category discovery: If the default cat=380 doesn't have Delhi Sultanate coins,
 * run with --discover to walk categories 370-410 and 780-810 and print titles.
 *
 * Usage:
 *   node scripts/scrape-delhi.js --test           # 5 coins, print samples
 *   node scripts/scrape-delhi.js --run             # full run
 *   node scripts/scrape-delhi.js --resume          # continue from checkpoint
 *   node scripts/scrape-delhi.js --discover        # find correct category ID
 *   node scripts/scrape-delhi.js --cat 380 --test  # test with a specific cat ID
 */
...
const OUTPUT_FILE    = path.join(SCRIPTS_DIR, '..', 'src', 'data', 'delhi_coins.json');
const PROGRESS_FILE  = path.join(SCRIPTS_DIR, 'delhi_progress.json');
...
async function main() {
  if (!TEST_MODE && !FULL_RUN && !RESUME_MODE && !CUSTOM_LIMIT && !DISCOVER && !CUSTOM_CAT && !COUNT_MODE) {
    ...
    console.log('  node scripts/scrape-delhi.js --resume         # resume from checkpoint');
    process.exit(0);
  }
  ...
  if (TEST_MODE) {
    console.log(`\nTest complete. Run with --run to scrape all Delhi Sultanate coins.`);

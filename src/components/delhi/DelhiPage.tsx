...
import { getDelhiCoins, getDelhiFilters } from '@/lib/coinsApi';
import type { CoinRow, DelhiCoinFilters, DelhiFilters } from '@/lib/coinsApi';
...
export default function DelhiPage({ locale }: { locale: string }) {
  ...
  const loadMints = useCallback(async (q: string): Promise<ComboOption[]> => {
    const all = dsFilters.current?.mints ?? [];
  ...
  const loadRulers = useCallback(async (q: string): Promise<ComboOption[]> => {
    const all = dsFilters.current?.rulers ?? [];
    const lower = q.toLowerCase();
    return all
      .filter(r => !q || r.en.toLowerCase().includes(lower) || (r.ar ?? '').includes(q))
      .slice(0, 50)
  ...
  const jsonLd = {
    ...
    name: isAr ? 'عملات سلطنة دلهي — أرابيزماتيكا' : 'Delhi Sultanate Coins — Arabismatica',
    url: `https://arabismatica.arabcollector.com/${locale}/delhi`,
  };

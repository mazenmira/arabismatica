...
function MintageTable({ data, locale }: { data: MintageEntry[]; locale: string }) {
  ...
  // Flag rows that share a year but differ in mintmark (highlight varieties)
  ...
  return (
    <div className="mt-1 mb-4">
      ...
      {/* Variety alert — show when mintmarks exist */}
      ...
      <div className="rounded-xl overflow-hidden border border-gold-700/25">
        <table className="w-full text-[12px]" dir={isAr ? 'rtl' : 'ltr'}>
          ...
          <tbody>
            {data.map((entry, idx) => {
              const isVarietyYear = entry.YearGregorian && yearCounts[entry.YearGregorian] > 1;
                ...
                >
                  ...
                  <td className="px-3 py-2.5 text-start">
                    <div className="flex flex-col gap-0.5">
                      {entry.YearHijri && (
                        <span className="font-amiri text-ink/80 text-[13px]">
                          {entry.YearHijri} هـ
                        </span>
                      )}
                      {entry.YearGregorian && (
                  ...
                  <td className="px-3 py-2.5 text-end">
                    {entry.MintageCount != null ? (
                      <span className={`font-mono text-[12px] ${isRare ? 'text-amber-300 font-bold' : 'text-ink/80'}`}>
                        {formatMintage(entry.MintageCount, locale)}
                      </span>
                    ) : (
                      <span className="text-ink/25 text-[11px]">
      ...
      {data.some(d => d.MintageCount != null) && (
        <div className="flex justify-end mt-1.5 gap-1 text-[10px] text-ink/30">
          ...
          <span className="text-ink/50 font-mono">
            {formatMintage(
              data.reduce((s, d) => s + (d.MintageCount ?? 0), 0),
              locale,
            )}
          </span>
        </div>
    ...
    >
      ...
      >
        ...
        {isValidImageUrl(src) ? (
          ...
          <div
            className="w-64 h-64 rounded-full flex items-center justify-center text-5xl font-amiri font-bold text-ink"
...
export default function CoinModal({
  ...
  // ── mintageData: new array schema OR legacy scalar ──
  const mintageData: MintageEntry[] = Array.isArray((coin as any).mintageData)
    ? (coin as any).mintageData
    : [];

  // Legacy scalar fallback
  const legacyMint: number | null = mintageData.length === 0 ? (coin.mint ?? null) : null;

  // Date range for header display
  const years = mintageData
    .map(d => d.YearGregorian)
        ...
        >
          ...
          <div className="px-5 py-5">
                  ...
                  <div key={side} className="flex flex-col items-center gap-2">
                    ...
                    >
                      {isValidImageUrl(src) ? (
                        ...
                        <div
                          className="w-[130px] h-[130px] rounded-full flex items-center justify-center font-amiri font-bold text-3xl text-ink"
            ...
            <div className={`mb-5 ${isAr ? 'text-right' : 'text-left'}`}>
              ...
              <div className="text-[12px] text-gold-600 font-medium mt-1">
                {coin.dyn} · {yearRangeLabel}
              </div>
            </div>

            {/* ── Mintage table (new) OR legacy scalar ── */}
              ...
              <SpecCard
                ...
                value={
                  coin.type === 'Commemorative'
                    ? isAr ? 'تذكارية' : 'Commemorative'
                    : isAr ? 'تداول' : 'Circulation'
                }
              />

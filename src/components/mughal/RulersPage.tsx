...
import { getMughalFilters } from '@/lib/coinsApi';

interface Ruler { en: string; ar: string | null; count: number }

const PER_PAGE = 40;

export default function MughalRulersPage({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    getMughalFilters().then(f => {
      setRulers(f.rulers.filter(r => r.en && r.en.trim() && r.count > 0));
      setLoading(false);
    ...
    <div className="max-w-5xl mx-auto px-4 py-8" dir={isAr ? 'rtl' : 'ltr'}>
      <div className="mb-8">
        <div className="flex items-center gap-2 text-[11px] text-gold-600/60 mb-3">
          ...
          <span>/</span>
          <span>{isAr ? 'العملات المغولية' : 'Mughal Coins'}</span>
          <span>/</span>
          <span className="text-gold-500">{isAr ? 'الحكام' : 'Rulers'}</span>
        </div>
        <h1 className="font-amiri text-3xl text-gold-300 mb-2">
          {isAr ? 'حكام الإمبراطورية المغولية' : 'Rulers of the Mughal Empire'}
        </h1>
        <p className="text-[13px] text-ink/50">
          {isAr ? '1526–1857م · انقر على اسم الحاكم لعرض عملاته' : '1526–1857 CE · Click a ruler to browse their coins'}
        </p>
      </div>
                ...
                {paged.map((ruler, i) => (
                  <tr key={ruler.en}
                    ...
                    <td className="px-3 py-2.5 text-center">
                      <Link
                        href={`/${locale}/mughal?ruler=${encodeURIComponent(ruler.en)}`}

...
export default function MughalMintsPage({ locale }: { locale: string }) {
    ...
    <div className="max-w-5xl mx-auto px-4 py-8" dir={isAr ? 'rtl' : 'ltr'}>
      <div className="mb-8">
        ...
        <h1 className="font-amiri text-3xl text-gold-300 mb-2">
          {isAr ? 'دور ضرب العملات المغولية' : 'Mughal Mint Cities'}
        </h1>
        <p className="text-[13px] text-ink/50">
          {isAr ? '1526–1857م · انقر على دار الضرب لعرض عملاتها' : '1526–1857 CE · Click a mint to browse its coins'}
        </p>
      </div>
                ...
                {paged.map((mint, i) => (
                  <tr key={mint.en}
                    ...
                    <td className="px-3 py-2.5 text-center">
                      <Link
                        href={`/${locale}/mughal?mint=${encodeURIComponent(mint.en)}`}
